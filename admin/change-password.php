<?php
	...
	if(strlen($_SESSION['clientmsaid']==0)){
		...
		if(isset($_POST['submit'])){
			$adminin = $_SESSION['clientmsaid'];
			$cpassword = md5($_POST['currentpassword']);
			$newpassword = md5($_POST['newpassword']);
			$sql = "SELECT ID FROM tbladmin WHERE ID=:adminid AND Password=:cpassword";
			$query= $dbh->prepare($sql);
			$query -> bindParam(':admin', $adminid, PDO::PARAM_STR);
			...
			if($query->rowCount() >0){
				...
				$change_pass = $dbh->prepare($con);
				$change_pass->bindParam(':adminid',$adminid,PDO::PARAM_STR);
				$change_pass->bindParam(':newpassword',$newpassword,PDO::PARAM_STR);
				$change_pass->execute();
